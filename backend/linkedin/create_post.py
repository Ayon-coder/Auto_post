...
class LinkedIn:
    ...
    def create_post(self):
        ...
        if elapsed > 7:
            if _VERBOSE:
                print(f"[SKIP] Skipping polling for LinkedIn link (Elapsed {elapsed:.1f}s)")
        ...
        for attempt in range(max_attempts):
            ...
            if _VERBOSE:
                print(f"[POLL] LinkedIn post {post_id} - Attempt {attempt+1}/{max_attempts} (Elapsed {elapsed:.1f}s)")
